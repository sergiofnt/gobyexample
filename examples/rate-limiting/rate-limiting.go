...
// дял підтримки необхідної якості роботи (і обмеження навантаження
//    з сторонніх джерел). Go дозволяє гарно реалізувати
// обмеження частоти за допомоги [горутин](goroutines), [каналів](channels)
...
func main() {
    ...
    // Розглянемо звичайний приклад обмеження частоти запитів.
    ...
    // значення кожні 200 мілісекунд - так звані "тіки",
    // це "регулятор" нашої системи обмеження.
    limiter := time.Tick(200 * time.Millisecond)

    // Тут відбувається блокування на отримання з каналу
    // `limiter` перед обробкою кожного запиту, ми чекатимемо
    // 200 мс і оброблятимето по одному запиту за цей період.
    for req := range requests {
        <-limiter
        fmt.Println("request", req, time.Now())
    }

    // Можливо, ми схочемо дозволяти невеликі "напливи" запитів,
    ...
    // Тепер, ми симулюємо 5 вхідних подій, 3 з яких
